...
LOG_DIR="/home/robert_tien/work/pachira/cnn-text-classification-tf/log/"
#pdb.set_trace()
FLAGS = tf.flags.FLAGS
FLAGS._parse_flags()
PATH="/home/robert_tien/work/word2vec/"
OUT_DIR="/home/robert_tien/work/pachira/cnn-text-classification-tf/data/"
now = datetime.datetime.now()
#tstr = now.strftime("%Y%m%d%H%M%S")
log_file = LOG_DIR+sys.argv[2]+".log"
...
if len(sys.argv) > 1 and sys.argv[1]=="bootstrap":
    x_text, y = dh.load_data_and_labels_bootstrap(sys.argv[3])
else:
...
for x in x_text:
    ...
    if len(x_) > maxlen:
        maxlen = len(x_)
        max_x = x
max_document_length_new = maxlen
"""
#vocab_processor = learn.preprocessing.VocabularyProcessor(max_document_length)
#x = np.array(list(vocab_processor.fit_transform(x_text)))
dictionary = dh.build_dict(x_text) #this should be deleted-no embedding layer. 
#res = dh.fit_transform(x_text, dictionary, max_document_length)
x = np.array(res)
...
x_train, x_dev = x_shuffled[:-len(y)/10], x_shuffled[-len(y)/10:]
y_train, y_dev = y_shuffled[:-len(y)/10], y_shuffled[-len(y)/10:]
#logging.info("Vocabulary Size: {:d}".format(len(vocab_processor.vocabulary_)))
logging.info("max_document_length: {:d}".format(max_document_length))
logging.info("Vocabulary Size: {:d}".format(len(dictionary)))
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        cnn = TextCNN(
            ...
            num_classes=y_train.shape[1], # 3 categories
        ...
        out_dir = os.path.join(OUT_DIR, "runs", timestamp)
        ...
        import json
        with open(os.path.join(out_dir, "vocab"), 'w') as fp:
